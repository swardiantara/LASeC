...
# samplings=( distance ) # on both sampling strategies
# num_samples=( 1 2 ) # effect of k values
...
initial_models=( all-MiniLM-L6-v2 all-MiniLM-L12-v2 all-distilroberta-v1 all-mpnet-base-v2 ) # effect of different initial models
samplings=( random )
num_samples=( 2 )
...
for model in "${initial_models[@]}"; do
    for k in "${num_samples[@]}"; do
        for sampling in "${samplings[@]}"; do
            python -m src.train_embedding --initial_model_path "$model" --sampling_strategy "$sampling" --k "$k" --push_embedding --batch_size 8
        done
    done
done
